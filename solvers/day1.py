...
# times the dial is left pointing at 0 after any rotation in the sequence.
...
from .utils import counter_with_wraparound, dial_counter

class solve(solver):
    def part1(self):
        instruction_list = self.parser.file_to_list()
        counter = counter_with_wraparound(0, 99, 50)
    ...
    def part2(self):
        instruction_list = self.parser.file_to_list()
        counter = dial_counter(0, 99, 50, [0])
        res = 0

        for instruction in instruction_list:
            direction, distance = instruction[0], int(instruction[1:])
            if direction == "L":
                val, passes = counter.change(-distance)
            else:
                val, passes = counter.change(distance)
